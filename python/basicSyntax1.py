...
###강사님 부연설명
# 컴터 메모리는 1byte 단위로 관리됨
# byte 단위로 칸 잡아먹음
# ex. int 형은 4byte, float 형은 8byte


# - 고유한 id(메모리 주소)
# - 타입 : 제품 종류(ex. int, str, float)
# - 값 : 제품 실제 내용물
### 위 3개 묶어서 객체(Objdect) 라고 함. (값 + 타입 + 주소) (이개념은 모호함 암기X)
### 주의 : 변수는 메모리 주소를 가지지 않음.

##강사님 부연설명 (정확한 객체 정의)
'''
객체 : 데이터 + 데이터와 관련 있는 알고리즘을 묶은 것
값과 그 값과 관련된 행동을 하나로 묶은 개념
파이썬에서 모든 데이터는 객체로 취급.
...
#강사님
'''
변수 할당하면 변수명에 객체로 값 저장됨
메모리상에 값들 저장. 재할당 전까지는 초기 저장값 기억
# 재할당 후에는 이전 값 기억x
'''
# 데이터
...
# start : 슬라이싱 시작할 인덱스 번호(포함o) 
# stop :슬라이싱 끝낼 인덱스 번호(포함x)
# step : 몇개씩 건너뛰며 값 가져올지 간격

# 슬라이싱 예제1
# 왜 끝에 값이 빠질까??
my_str = 'hello'
my_str[2:4] # ll 
# n번 해버리면 n-1 이라고 표현해야 (갯수 맞음, 컴터는 0부터 세고, 사람은 1부터 셈)
...
my_str = 'hello'
my_str[::2] # hlo
# 처음부터 끝까지 2칸 간격으로 값을 가져옴 (슬라이싱 영역 잘 생각하기 |h|e|l|l|o|)
# 영역이 처음부터 끝이니까, 0번째 값 가져온 다음, 그뒤로 2칸 간격 적용
...
##### 참고 #####
# int 진법 표현(10진법)
# 10(2진수)은 10진수로 2임 (1 * 2^1 + 0 * 2^0)
print(0b10) ## int 10 -> 2진수 변환하려면 ob붙이기
# 30(8진수)는 10진수로 24임
print(0o30) ## 8진수 -> 10진수 : 0o

print(0x10) # 16진수 -> 10진수 : 0x
# 반올림 오차 
...
# 구글에 파이썬 튜터 검색 - 파이썬 링크 클릭
# 실행 전에 선택 render all objects on the heep (모든 객체 그림그려줌)
# 입력창에 코드 입력 후 실행
'''
heep : 메모리 영역 중에서 객체를 저장하는 영역
'''
